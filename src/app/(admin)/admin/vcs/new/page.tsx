...
export default function NewVCPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error creating VC firm:', err);
      setError(err.message || 'Failed to create VC firm.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Add New VC Firm</h2>
          <p className="text-muted-foreground">
            Create a new venture capital firm listing.
          </p>
        </div>
      </div>
      ...
      {success && (
        <div className="rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">
          VC firm created successfully! Redirecting...
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <Button type="submit" disabled={saving} className="gap-2">
            ...
            {saving ? 'Creating...' : 'Create VC Firm'}
          </Button>
        </div>
      </form>
    </div>
